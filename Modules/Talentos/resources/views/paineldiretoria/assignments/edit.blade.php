...
<div class="mx-auto max-w-2xl space-y-8 pb-10">
    ...
    <div>
        ...
        <h1 class="mt-3 text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">Editar atribuição</h1>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Atualize função, evento, estado ou notas internas.</p>
    </div>
    ...
    <form method="post" action="{{ route('diretoria.talentos.assignments.update', $assignment) }}" class="space-y-6 rounded-2xl border border-gray-200/90 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-8">
        @csrf
        @method('PUT')
