...
<div class="mx-auto max-w-7xl space-y-8 pb-16 font-sans animate-fade-in">
    ...
    @include('blog::paineldiretoria.partials.page-hero', [
        'kicker' => 'Edição',
        'title' => 'Editar post',
        'lead' => 'Atualize texto, imagens, SEO e status. Alterações em rascunho não aparecem no site até a publicação.',
        'iconName' => 'pen-to-square',
        'crumbs' => [
            ['label' => 'Diretoria', 'url' => route('diretoria.dashboard')],
            ['label' => 'Blog', 'url' => blog_admin_route('index')],
            ['label' => Str::limit($post->title, 32), 'url' => blog_admin_route('show', $post->id)],
            ['label' => 'Editar', 'url' => null],
        ],
        'actions' => view('blog::paineldiretoria.partials.hero-actions-post-edit', ['post' => $post])->render(),
    ])

<!-- Form -->
<form action="{{ blog_admin_route('update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                ...
                @include('blog::admin.partials.bible-assist')

                <div class="blog-editor-wrapper">
                    <div id="blog-content-editor" class="h-96" data-upload-url="{{ blog_admin_route('upload-image') }}">{!! old('content', $post->content) !!}</div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                <x-icon name="folder-tree" class="w-5 h-5 mr-2 text-emerald-600" />
                Categoria
            </h3>
        ...
        @include('blog::partials.post-tags-field', ['tags' => $tags, 'post' => $post])
    </div>
</div>
</form>
</div>
...
function saveRedaction() {
    ...
    fetch(@json(blog_admin_route('redact-image')), {
        method: 'POST',
        headers: {
