    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 md:pb-6 border-b border-slate-200 dark:border-slate-800">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3 mb-2">
                <span class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-emerald-600 shadow-lg">
                    <x-module-icon module="Avisos" class="h-6 w-6 text-white" />
                </span>
                Avisos e comunicados
            </h1>
            <p class="text-sm md:text-base text-slate-600 dark:text-slate-400">
                Comunicados oficiais da JUBAF para a tua liderança e congregação.
            </p>
        </div>
        <form method="get" action="{{ route('lideres.avisos.index') }}" class="flex w-full sm:w-auto gap-2">
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
        <ul class="divide-y divide-slate-200 dark:divide-slate-800">
            @forelse($avisos as $aviso)
                <li>
                    <a href="{{ route('lideres.avisos.show', $aviso) }}" class="flex flex-col gap-3 p-5 md:p-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        ...
                        @endif
                        <span class="inline-flex items-center gap-1 text-sm font-semibold text-emerald-600 dark:text-emerald-400">Abrir <x-icon name="arrow-right" class="w-4 h-4" /></span>
                    </a>
                </li>
            @empty
