{{--
    Navegação interna — Membros da diretoria no site público.
...
@endphp
<nav class="rounded-2xl border border-gray-200/90 bg-white/90 p-1.5 shadow-sm backdrop-blur-sm dark:border-slate-700 dark:bg-slate-900/80" aria-label="Secções de membros da diretoria">
    <div class="flex flex-wrap gap-1">
        <a href="{{ route('diretoria.board-members.index') }}" class="{{ $linkBase }} {{ $listaActive ? $linkActive : $linkIdle }}">
            <x-icon name="users" class="h-4 w-4 shrink-0 opacity-90" style="duotone" />
            Lista
        </a>
        @can('create', \App\Models\BoardMember::class)
            <a href="{{ route('diretoria.board-members.create') }}" class="{{ $linkBase }} {{ $active === 'nova' ? $linkActive : $linkIdle }}">
                <x-icon name="user-plus" class="h-4 w-4 shrink-0 opacity-90" style="duotone" />
                Novo membro
            </a>
        @endcan
    </div>
