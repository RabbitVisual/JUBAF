 ...
 * JUBAF2026: Todas as escritas em DB::transaction com log em audit_financial_logs.
 */
class TreasuryApiService
{
    ...
    public function getDashboardStats(User|int $userOrId): array
    {
        ...
        $recentEntries = FinancialEntry::with(['user', 'campaign'])
    ...
    {
        $query = FinancialEntry::with(['user', 'campaign', 'payment']);
    ...
    private const LEGACY_CATEGORIES = [
        'tithe', 'offering', 'donation', 'campaign',
    ...
    private const SLUG_TO_LEGACY_CATEGORY = [
        ...
        'construction_fund' => 'campaign', 'donation' => 'donation',
    ...
    {
        return FinancialEntry::with(['user', 'campaign', 'payment'])->findOrFail($id);
    }

    /**
     * Create entry. Permission: canCreateEntries. Updates campaign/goal current amounts.
     * JUBAF2026: DB::transaction + audit log; expense_status = pending for expenses; category_id from slug.
     */
    public function createEntry(array $data, User|int $userOrId): FinancialEntry
    {
        ...
        $entry = DB::transaction(function () use ($data, $user) {
            ...
            // Lógica de aprovação automática simplificada para JUBAF
            $limit = (float) \App\Models\Setting::get('treasury_auto_approve_limit', 1000);
            
            if ($entry->type === 'expense' && (float) $entry->amount > $limit) {
                 // Para JUBAF, despesas acima do limite podem ser notificadas aos Tesoureiros/Presidente
                 // Futuramente implementar fluxo de aprovação da diretoria aqui
            }
        ...
        return $entry->load(['user', 'campaign', 'payment', 'financialCategory', 'fund']);
    }

    /**
     * Update entry. Permission: canEditEntries. JUBAF2026: DB::transaction + audit log.
     */
    public function updateEntry(FinancialEntry $entry, array $data, User|int $userOrId): FinancialEntry
    {
        ...
        return $entry->fresh(['user', 'campaign', 'payment', 'financialCategory', 'fund']);
    }

    /**
     * Delete entry (soft). Permission: canDeleteEntries. JUBAF2026: DB::transaction + audit log.
     */
    ...
    public function reverseEntry(FinancialEntry $entry, User|int $userOrId): FinancialEntry
    {
        ...
        return $reversal->load(['user', 'campaign', 'payment', 'financialCategory', 'fund', 'reversalOf']);
    }

    /**
     * Import payment into Treasury. Permission: canCreateEntries. JUBAF2026: delegates to createEntry (transaction + audit).
     */
     ...
     * Permission: canViewReports + (treasury authorized).
     */
    ...
    /**
     * Mark a monthly closing as ready for assembly (statutory approval flow).
     */
    public function markClosingReadyForAssembly(TreasuryMonthlyClosing $closing, User|int $userOrId, ?string $notes = null): TreasuryMonthlyClosing
    {
        ...
        // Apenas Admin ou Tesoureiros/Presidente da JUBAF podem aprovar fechamentos
        $isAuthorized = $user->isAdmin() || $user->isTreasurer() || $user->isPresident();

        if (! $isAuthorized) {
            abort(403, 'Apenas a diretoria financeira ou presidente podem aprovar fechamentos mensais.');
        }
            ...
            if (class_exists(\Modules\Notifications\App\Services\InAppNotificationService::class)) {
                try {
                    ...
                    app(\Modules\Notifications\App\Services\InAppNotificationService::class)->sendToAdmins(
                        ...
                        "O balancete de {$monthLabel} foi marcado como pronto para apresentação em assembleia.",
                        [
                            'type' => 'success',
                            'priority' => 'normal',
                        ]
    ...
    /**
     * Plano Cooperativo: percentual sobre base (dízimos, dízimos+ofertas ou receita total). JUBAF2026.
     */
    ...
    /**
     * Options for entry forms: campaigns, goals, funds, categories, payments (for import). JUBAF2026.
     */
