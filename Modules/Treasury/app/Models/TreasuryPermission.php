...
class TreasuryPermission extends Model
{
    ...
    public static function forUserOrAdmin(User $user): self
    {
        ...
        if (method_exists($user, 'canAccess') && $user->canAccess('gerenciar_financeiro')) {
            return self::adminDefault();
        }
        if (MemberPanelAccess::hasActiveGrant($user, MemberPanelAccess::MODULE_TREASURY)) {
            return self::grantViewerDefault();
        }
    ...
    /**
     * Acesso só leitura / relatórios para utilizadores com grant explícito do presidente.
     */
    public static function grantViewerDefault(): self
    {
        $p = new self([
