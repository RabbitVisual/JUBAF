...
class ReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $monthlyClosing = null;
        $canApproveMonthlyClosing = false;
        ...
        if ($user) {
            ...
            if ($isSingleMonth) {
                $allowStatutoryApproval = (bool) \App\Models\Settings::get('church_council_allow_admin_approval', true);

                $isAdminOrStatutory = $user->hasRole(['admin', 'tesoureiro', 'presidente']);

                $canApproveMonthlyClosing = $allowStatutoryApproval && $isAdminOrStatutory;
        ...
        return view('treasury::admin.reports.index', [
            ...
            'canApproveMonthlyClosing' => $canApproveMonthlyClosing,
        ]);
    }

    /**
     * Aprovar um fechamento mensal para apresentação em assembleia (balancete consolidado).
     */
    ...
    public function exportExcel(Request $request)
    {
        ...
        $entries = FinancialEntry::with(['user', 'campaign'])
    ...
    /**
     * Comprovante anual de contribuição (JUBAF). Por member_id e ano. Membro só pode ver o próprio; tesoureiro/admin pode ver qualquer.
     */
