...
@section('content')
    <div class="space-y-8">
        ...
        @if(isset($monthlyClosing) && $monthlyClosing)
            <div class="mt-4 bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-emerald-300/70 dark:border-emerald-500/60 px-5 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        <div class="p-1.5 rounded-lg bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">
                            <x-icon name="badge-check" class="w-4 h-4" />
                        </div>
                        <span class="text-[10px] font-black uppercase tracking-widest text-emerald-700 dark:text-emerald-300">
                    ...
                    <p class="text-xs text-gray-700 dark:text-gray-300">
                        ...
                        @if($monthlyClosing->ready_for_assembly)
                            <span class="text-emerald-600 dark:text-emerald-400 font-semibold">
                                Consolidado e Pronto para Assembleia
                            </span>
                        @else
                            <span class="text-amber-600 dark:text-amber-400 font-semibold">
                                Pendente de Consolidação Final
                            </span>
                        @endif
                    </p>
                </div>

                @if(! $monthlyClosing->ready_for_assembly && ! empty($canApproveMonthlyClosing))
                    <div class="flex items-center gap-2">
                        ...
                        >
                            ...
                            <span>Consolidar para Assembleia</span>
                        </button>
                    </div>
                @endif
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            btn.addEventListener('click', async function () {
                if (!confirm('Confirmar consolidação deste balancete e marcar como pronto para assembleia?')) {
                    return;
                }
                ...
                window.dispatchEvent(new CustomEvent('loading-overlay:show', { detail: { message: 'Consolidando balancete...' } }));
