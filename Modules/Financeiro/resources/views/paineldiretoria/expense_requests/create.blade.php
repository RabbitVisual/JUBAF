...
@section('title', 'Novo pedido de reembolso')
...
<div class="mx-auto max-w-2xl space-y-8 pb-10">
    ...
    <div>
        ...
        <h1 class="mt-4 text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">Novo pedido</h1>
        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">O pedido fica em rascunho até submeter. Anexe comprovativo quando existir (PDF ou imagem).</p>
    </div>

    <form action="{{ route('diretoria.financeiro.expense-requests.store') }}" method="post" enctype="multipart/form-data" class="space-y-6">
        @csrf
        <div class="overflow-hidden rounded-2xl border border-gray-200/90 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
            ...
            <div class="space-y-5 p-6">
                <div>
                    ...
                    <input type="number" step="0.01" min="0.01" name="amount" value="{{ old('amount') }}" required class="{{ $in }} tabular-nums" inputmode="decimal">
                ...
                <div>
                    ...
                    <textarea name="justification" rows="5" required class="{{ $in }} min-h-[8rem]">{{ old('justification') }}</textarea>
                    @error('justification')<p class="mt-1.5 text-xs font-medium text-rose-600 dark:text-rose-400">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="{{ $lb }}">Comprovativo (opcional)</label>
            ...
            <div class="flex flex-wrap items-center justify-end gap-3 border-t border-gray-100 bg-gray-50/80 px-6 py-4 dark:border-slate-700 dark:bg-slate-900/40">
                ...
                <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white shadow-md shadow-emerald-600/20 transition hover:bg-emerald-700">
                    <x-icon name="floppy-disk" class="h-4 w-4" style="solid" />
                    Guardar rascunho
                </button>
            </div>
        </div>
