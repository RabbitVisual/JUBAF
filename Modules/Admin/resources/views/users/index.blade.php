    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 relative overflow-hidden group">
                ...
                <div class="relative flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Igreja local</p>
                        <p class="mt-1 text-3xl font-black text-cyan-600 dark:text-cyan-400">{{ $stats['with_church'] }}</p>
                    </div>
                    <div class="w-12 h-12 rounded-2xl bg-cyan-100 dark:bg-cyan-900/30 flex items-center justify-center text-cyan-600 dark:text-cyan-400">
                        <x-icon name="place-of-worship" class="w-6 h-6" />
                    </div>
                </div>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 relative overflow-hidden group">
        ...
        @if(request()->filled('church_id') && isset($churches))
            @php
                $filterChurch = $churches->firstWhere('id', (int) request('church_id'));
            @endphp
            @if($filterChurch)
                <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-blue-200 dark:border-blue-900/50 bg-blue-50/80 dark:bg-blue-900/20 px-4 py-3">
                    <p class="text-sm text-gray-700 dark:text-gray-300">
                        <span class="font-bold">Igreja local:</span> {{ $filterChurch->name }}
                    </p>
                    <a href="{{ route('admin.churches.show', $filterChurch) }}" class="text-sm font-bold text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center gap-1">
                        <x-icon name="place-of-worship" class="w-4 h-4" />
                        Ver cadastro da igreja
                    </a>
                </div>
            @endif
        @endif
            ...
            <div class="relative">
                <form action="{{ route('admin.users.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    ...
                    <div class="md:col-span-2 flex flex-wrap items-end gap-2">
                        ...
                        @if(request()->hasAny(['search', 'role_id', 'is_active', 'church_id']))
                    ...
                    <div class="md:col-span-12">
                        <label for="church_id" class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Igreja local (ASBAF)</label>
                        <select name="church_id" id="church_id" class="block w-full max-w-xl px-3 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 text-sm">
                            <option value="">Todas as igrejas</option>
                            @foreach($churches as $c)
                                <option value="{{ $c->id }}" {{ (string) request('church_id') === (string) $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    @endif
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Igreja local</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach($users as $user)
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-300 max-w-[14rem] truncate" title="{{ $churchLabel }}">
                                        {{ $churchLabel }}
                                    </td>
