...
@endif
    ...
    @if ($homepageSettings['show_publications'] && ($publicationsMinutes->isNotEmpty() || $publicationsComms->isNotEmpty()))
        <section id="publicacoes" class="py-24 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 transition-colors duration-200">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col md:flex-row justify-between items-end mb-12 gap-4">
                    <div class="text-center md:text-left">
                        <span class="text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wider text-sm mb-2 block">Transparência</span>
                        <h2 class="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-white">
                            {{ $homepageSettings['publications_title'] }}
                        </h2>
                        @if (!empty($homepageSettings['publications_intro']))
                            <p class="mt-3 text-gray-600 dark:text-gray-400 max-w-2xl">{{ $homepageSettings['publications_intro'] }}</p>
                        @endif
                    </div>
                    <a href="{{ route('public.transparency.index') }}" class="text-emerald-600 hover:text-emerald-800 dark:text-emerald-400 font-semibold flex items-center shrink-0">
                        Arquivo completo <x-icon name="arrow-right" class="w-4 h-4 ml-1" />
                    </a>
                </div>

                <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($publicationsMinutes as $minute)
                        <a href="{{ route('public.transparency.minute', $minute->slug) }}" class="block group bg-gray-50 dark:bg-gray-800 rounded-3xl shadow-sm hover:shadow-xl border border-gray-100 dark:border-gray-700 p-6 transition-all">
                            <span class="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">Ata</span>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white mt-2 group-hover:text-emerald-600 dark:group-hover:text-emerald-400">
                                {{ $minute->assembly?->title ?? 'Assembleia' }}
                            </h3>
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">
                                {{ $minute->published_at?->translatedFormat('d M Y') }}
                            </p>
                        </a>
                    @endforeach
                    @foreach ($publicationsComms as $comm)
                        <a href="{{ route('public.transparency.communication', $comm->slug) }}" class="block group bg-gray-50 dark:bg-gray-800 rounded-3xl shadow-sm hover:shadow-xl border border-gray-100 dark:border-gray-700 p-6 transition-all">
                            <span class="text-xs font-bold uppercase tracking-wider text-purple-600 dark:text-purple-400">Comunicado</span>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white mt-2 group-hover:text-emerald-600 dark:group-hover:text-emerald-400">
                                {{ $comm->title }}
                            </h3>
                            @if ($comm->summary)
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-2 line-clamp-2">{{ $comm->summary }}</p>
                            @endif
                        </a>
                    @endforeach
                </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center mb-16">
                    ...
                    <p class="text-blue-100 max-w-2xl mx-auto">
                        JUBAF — jovens batistas em articulação com a ASBAF, igrejas locais e UNIJOVEM
                    </p>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
                    ...
                    <!-- Igrejas locais (ASBAF) -->
                    <div class="text-center">
                        <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
                            <x-icon name="church-alt" class="w-16 h-16 text-white mx-auto mb-4" />
                            <div class="text-4xl font-bold mb-2">{{ $statistics['churches'] }}</div>
                            <div class="text-blue-100">Igrejas locais</div>
                        </div>
                    </div>
