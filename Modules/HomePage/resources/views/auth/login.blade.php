        ...
        <div class="w-full max-w-5xl grid lg:grid-cols-2 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden z-10">
            ...
            <div class="p-8 sm:p-12 lg:p-16 flex flex-col justify-center order-2 lg:order-1">
                ...
                @if(app()->environment('local', 'development', 'dev'))
                    @php
                        $devDemoAccounts = config('dev_demo_users.accounts', []);
                    @endphp
                    <div class="mt-10 pt-8 border-t border-gray-100 dark:border-gray-800">
                        <div class="text-center mb-2">
                            <span class="px-3 py-1 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 text-[10px] font-bold uppercase tracking-wider rounded-full">Ambiente de desenvolvimento</span>
                        </div>
                        <p class="text-center text-[11px] text-gray-500 dark:text-gray-400 mb-4">
                            Logins demo por perfil (RBAC JUBAF). Use os campos para entrar manualmente ou o botão para login rápido.
                            Se faltar utilizador, execute <code class="text-purple-600 dark:text-purple-400">php artisan db:seed --class=DemoUsersSeeder</code>.
                        </p>
                        <div class="max-h-[min(22rem,55vh)] overflow-y-auto pr-1 -mr-1 space-y-2 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-3">
                            @foreach($devDemoAccounts as $demo)
                                <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-800/50 p-3 flex flex-col gap-2">
                                    <div>
                                        <p class="text-xs font-bold text-gray-900 dark:text-white">{{ $demo['label'] ?? $demo['slug'] }}</p>
                                        <p class="text-[10px] font-mono text-gray-600 dark:text-gray-400 break-all mt-0.5">{{ $demo['email'] }}</p>
                                        <p class="text-[10px] font-mono text-gray-500 dark:text-gray-500">Senha: {{ $demo['password'] }}</p>
                                    </div>
                                    <form action="{{ route('quick-login') }}" method="POST" class="mt-auto">
                                        @csrf
                                        <input type="hidden" name="role" value="{{ $demo['slug'] }}">
                                        <button type="submit" class="w-full py-2 px-3 bg-white dark:bg-gray-800 border border-purple-200 dark:border-purple-900/40 rounded-lg text-[10px] font-bold text-purple-700 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
                                            Entrar como {{ $demo['label'] ?? $demo['slug'] }}
                                        </button>
                                    </form>
                                </div>
                            @endforeach
