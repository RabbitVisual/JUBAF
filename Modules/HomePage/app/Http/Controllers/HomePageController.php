...
class HomePageController extends Controller
{
    ...
    public function index()
    {
        ...
        $publicationsMinutes = collect();
        $publicationsComms = collect();
        try {
            if (class_exists(Minute::class)) {
                $publicationsMinutes = Minute::query()
                    ->published()
                    ->with('assembly')
            ...
            if (class_exists(OfficialCommunication::class)) {
                $publicationsComms = OfficialCommunication::query()
                    ->published()
                    ->orderByDesc('published_at')
                    ->limit(3)
                    ->get();
            ...
            \Log::debug('Homepage publications: '.$e->getMessage());
        }
    ...
    private function getDynamicStatistics()
    {
        ...
        $churchesCount = (int) Settings::get('homepage_stats_churches', 0);
        if ($churchesCount <= 0) {
            try {
                if (class_exists(\Modules\Church\Models\Church::class)) {
                    $churchesCount = (int) \Modules\Church\Models\Church::where('is_active', true)->count();
                }
            } catch (\Exception $e) {
                $churchesCount = 0;
            }
    ...
    private function getHomepageSettings()
    {
        return [
            ...
            'hero_subtitle' => Settings::get('homepage_hero_subtitle', 'Jovens das igrejas batistas arroladas à ASBAF — congresso, assembleias e serviço em Feira de Santana e região.'),
            ...
            'publications_intro' => Settings::get('homepage_publications_intro', 'Atas de assembleias e comunicados oficiais da diretoria.'),
            ...
            'contact_email' => Settings::get('site_email', 'contato@jubaf.org.br'),
