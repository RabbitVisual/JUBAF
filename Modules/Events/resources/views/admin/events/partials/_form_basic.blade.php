...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="md:col-span-2">
            <label for="institutional_track" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Calendário institucional JUBAF
            </label>
            <select name="institutional_track" id="institutional_track"
                class="block w-full max-w-lg rounded-lg border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:text-white sm:text-sm">
                <option value="">— Não classificado —</option>
                @foreach([
                    'conjubaf' => 'CONJUBAF / Congresso geral',
                    'congresso_lideres' => 'Congresso de líderes',
                    'start_jubaf' => 'Start JUBAF',
                    'encontro_setores' => 'Encontro de setores',
                    'jubaf_na_estrada' => 'JUBAF na estrada',
                    'outro' => 'Outro',
                ] as $val => $label)
                    <option value="{{ $val }}" @selected(old('institutional_track', $ev?->institutional_track) === $val)>{{ $label }}</option>
                @endforeach
            </select>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Opcional: alinha relatórios ao plano anual da diretoria.</p>
        </div>
