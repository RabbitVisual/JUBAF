            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="space-y-3">
                    @if($registration->status === 'pending')
                        <form action="{{ route('admin.events.registrations.confirm', [$event, $registration]) }}" method="POST">
                            @csrf
                    ...
                    @if($registration->status !== 'cancelled')
                        <form action="{{ route('admin.events.registrations.cancel', [$event, $registration]) }}" method="POST" onsubmit="return confirm('{{ __('events::messages.cancel_confirm') }}');">
                            @csrf
