        ...
        <ul class="space-y-2">
            @foreach ($convocations as $c)
                <li class="rounded-lg border border-gray-200 p-3 dark:border-gray-700">
                    <a href="{{ route($namePrefix.'.convocatorias.show', $c) }}" class="font-medium text-blue-600 hover:underline dark:text-blue-400">{{ $c->title }}</a>
                    <span class="ml-2 text-sm text-gray-500 dark:text-gray-400">{{ $c->assembly_at->format('d/m/Y H:i') }}</span>
                </li>
            @endforeach
        </ul>
        {{ $convocations->links() }}
