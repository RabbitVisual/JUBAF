            ...
            >
                ...
                <div class="relative">
                    <p class="text-xs font-bold uppercase tracking-wider text-blue-100/90">
                        {{ $convocation->assembly_at->format('d/m/Y · H:i') }}
                    </p>
...
@else
    <div class="max-w-4xl space-y-4">
        <a href="{{ route($namePrefix.'.convocatorias.index') }}" class="text-sm text-blue-600 hover:underline dark:text-blue-400">← Convocatórias</a>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $convocation->title }}</h1>
        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $convocation->assembly_at->format('d/m/Y H:i') }}</p>
        <div class="prose rounded-xl border border-gray-200 bg-white p-6 dark:prose-invert dark:border-gray-700 dark:bg-gray-800">{!! $convocation->body !!}</div>
    </div>
@endif
