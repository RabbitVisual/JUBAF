<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900/80">
    <ul class="divide-y divide-gray-200 dark:divide-gray-800">
        @forelse ($documents as $d)
            <li class="flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:justify-between md:px-6">
                ...
                @can('download', $d)
                    <a href="{{ route($namePrefix.'.documentos.download', $d) }}" class="inline-flex shrink-0 items-center gap-2 text-sm font-semibold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300">
    ...
    @if ($documents->hasPages())
        <div class="border-t border-gray-200 bg-gray-50/80 px-5 py-4 dark:border-gray-800 dark:bg-gray-900/50 md:px-6">{{ $documents->links() }}</div>
    @endif
</div>