...
<div class="mx-auto max-w-7xl space-y-8 pb-10">
    ...
    <div class="border-b border-gray-200 pb-6 dark:border-slate-700">
        ...
        <h1 class="mt-1 text-2xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-3xl">Editar reunião</h1>
    </div>

    <form action="{{ route($routePrefix.'.update', $meeting) }}" method="POST" class="mx-auto max-w-2xl space-y-6 rounded-2xl border border-gray-200/90 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        @csrf @method('PUT')
