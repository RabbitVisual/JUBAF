...
@section('content')
    <div class="space-y-6">
        ...
        <form action="{{ route('admin.churches.update', $church) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2 col-span-1 md:col-span-2">
                        ...
                        <select name="unijovem_representative_user_id"
                            ...
                            @foreach ($users as $u)
                                <option value="{{ $u->id }}" @selected(old('unijovem_representative_user_id', $church->unijovem_representative_user_id) == $u->id)>{{ $u->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
