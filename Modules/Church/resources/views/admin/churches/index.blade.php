...
@section('content')
    <div class="space-y-8">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <span class="px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-300 text-xs font-bold uppercase tracking-wider">Associação</span>
                            <span class="px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-400/30 text-indigo-300 text-xs font-bold uppercase tracking-wider">Igrejas</span>
                        </div>
                        <h1 class="text-3xl md:text-4xl font-black tracking-tight mb-2">Igrejas & Lideranças</h1>
                        <p class="text-gray-300 max-w-xl">Gerencie as igrejas, congregações, grupos de jovens (UNIJOVEM) e líderes vinculados à JUBAF.</p>
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 shadow-sm">
            <form action="{{ route('admin.churches.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
                <div class="w-full md:flex-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            class="block w-full pl-10 pr-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                ...
                <div class="w-full md:w-48">
                    ...
                    <div class="relative">
                        <select name="sector" class="block w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all appearance-none">
                ...
                <div class="w-full md:w-48">
                    ...
                    <div class="relative">
                        <select name="status" class="block w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all appearance-none">
                ...
                <div class="flex gap-2 w-full md:w-auto">
                    ...
                    @if(request()->anyFilled(['search', 'sector', 'status']))
                        <a href="{{ route('admin.churches.index') }}" class="px-4 py-2.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 font-bold rounded-xl hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors tooltip" title="Limpar Filtros">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($churches as $church)
                <div class="group bg-white dark:bg-gray-800 rounded-3xl shadow-sm hover:shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col transition-all duration-300 relative">
                    ...
                    <!-- Header/Cover -->
                    <div class="relative h-36 overflow-hidden bg-gradient-to-br from-indigo-500/10 to-blue-600/10 flex items-center justify-center p-6 border-b border-gray-100 dark:border-gray-700">
                        @if ($church->logo_path)
                        ...
                        @else
                            <x-icon name="place-of-worship" style="duotone" class="w-16 h-16 text-indigo-500/30 group-hover:scale-110 transition-transform duration-700" />
                        @endif
                    </div>

                    <!-- Body -->
                    <div class="p-6 flex-1 flex flex-col">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white line-clamp-2 leading-tight mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                            {{ $church->name }}
                        </h3>
                        ...
                        <div class="space-y-3 mt-4 flex-1">
                            @if ($church->unijovem_name)
                                <div class="flex items-start gap-3">
                                    <div class="mt-0.5 p-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400">
                        ...
                        <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
                            <div class="flex gap-2">
                                <a href="{{ route('admin.churches.show', $church) }}" class="p-2 text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-all tooltip" title="Visualizar">
                                    <x-icon name="eye" style="duotone" class="w-4 h-4" />
                                </a>
                                <a href="{{ route('admin.churches.edit', $church) }}" class="p-2 text-gray-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-xl transition-all tooltip" title="Editar">
                                    <x-icon name="pencil" style="duotone" class="w-4 h-4" />
                                </a>
                            </div>
                            <form action="{{ route('admin.churches.destroy', $church) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta igreja? Todos os dados vinculados podem ser afetados.')">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <div class="col-span-full flex flex-col items-center justify-center py-20 px-8 bg-white dark:bg-gray-800 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                    ...
                    <a href="{{ route('admin.churches.create') }}" class="inline-flex items-center px-6 py-3 bg-gray-900 dark:bg-blue-600 text-white rounded-xl font-bold hover:bg-gray-800 dark:hover:bg-blue-700 transition-all">
