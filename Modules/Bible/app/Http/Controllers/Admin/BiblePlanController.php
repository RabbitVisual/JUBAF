...
class BiblePlanController extends Controller
{
    ...
    public function index()
    {
        ...
        return view($this->bibleView('plans.index'), compact('plans'));
    }

    public function create()
    {
        return view($this->bibleView('plans.create'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'type' => 'required|in:manual,sequential,chronological,thematic',
            'reading_mode' => 'required|in:digital,physical_timer',
            'cover_image' => 'nullable|image|max:2048',
            'is_featured' => 'sometimes|boolean',
            'is_church_plan' => 'sometimes|boolean',
            'complexity' => 'nullable|string|in:light,moderate,intense',
        ...
        $plan = BiblePlan::create([
            ...
            'is_church_plan' => $request->boolean('is_church_plan'),
            'complexity' => $request->input('complexity'),
            'template_key' => $request->input('template_key'),
        ]);

        if (in_array($request->type, ['sequential', 'thematic'], true)) {
            return redirect()->to($this->bibleRoute('plans.generate', $plan->id));
        }

        return redirect()->to($this->bibleRoute('plans.show', $plan->id));
    }
    ...
    public function show($id)
    {
        ...
        return view($this->bibleView('plans.show'), compact('plan', 'books'));
    }
    ...
    public function edit($id)
    {
        ...
        return view($this->bibleView('plans.edit'), compact('plan'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect()->to($this->bibleRoute('plans.show', $plan->id))->with('success', 'Plano atualizado com sucesso!');
    }
    ...
    public function generator($id)
    {
        ...
        return view($this->bibleView('plans.generator'), compact('plan', 'books', 'versions'));
    }
    ...
    public function processGeneration(Request $request, $id)
    {
        ...
        $rules = [
            'bible_version_id' => ['nullable', 'exists:bible_versions,id'],
            'order_type' => ['nullable', 'string', 'in:canonical,historical,christ_centered'],
            'book_ids' => ['sometimes', 'array'],
            'book_ids.*' => ['integer', 'exists:books,id'],
        ];
        if ($plan->type !== 'manual') {
            $rules['bible_version_id'] = ['required', 'exists:bible_versions,id'];
        }
        $request->validate($rules);

        $versionId = $request->filled('bible_version_id') ? (int) $request->input('bible_version_id') : null;
    ...
    public function editDay($planId, $dayId)
    {
        ...
        return view($this->bibleView('plans.edit-day'), compact('plan', 'day', 'books', 'versions'));
    }
