...
class BibleController extends Controller
{
    ...
    {
        $versionId = $request->filled('version_id') ? (int) $request->validated('version_id') : null;
        $result = $this->bibleApi->findByReference($request->validated('ref'), $versionId);
        if ($result === null) {
    ...
    public function search(SearchBibleRequest $request): JsonResponse
    {
        ...
        $versionAbbr = $validated['version'] ?? null;
        $versionId = null;
        if ($versionAbbr !== null && $versionAbbr !== '') {
            $versionId = BibleVersion::query()->where('abbreviation', $versionAbbr)->value('id');
        }

        // Try exact reference first
        $findResult = $this->bibleApi->findByReference($query, $versionId ? (int) $versionId : null);
        if ($findResult !== null) {
            return response()->json([
                'data' => [
                    'type' => 'exact',
        ...
        $verses = $this->bibleApi->search($query, $limit, $versionAbbr)->load('chapter.book');
