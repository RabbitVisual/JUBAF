...
@section('content')
    <div class="space-y-8 pb-12 -mt-2">
        <header class="sticky top-0 z-20 -mx-4 px-4 md:mx-0 md:px-0 md:static bg-stone-50/95 dark:bg-stone-950/95 md:bg-transparent backdrop-blur md:backdrop-none border-b border-stone-200/80 dark:border-stone-800 md:border-0 pb-4 md:pb-0">
            <div class="flex items-start gap-4">
                <a href="{{ route('jovens.bible.read', $version->abbreviation) }}"
                   class="mt-1 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 text-stone-500 hover:text-teal-700 hover:border-teal-400/50 transition-colors"
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <h1 class="text-2xl md:text-3xl font-bold text-stone-900 dark:text-stone-50 tracking-tight">{{ $book->name }}</h1>
        ...
        <div class="relative overflow-hidden rounded-3xl border border-teal-200/60 dark:border-teal-900/50 bg-gradient-to-br from-teal-600 to-teal-800 text-white shadow-lg shadow-teal-900/10">
            ...
            <div class="relative px-6 py-8 md:px-10 md:py-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
                <div>
                    <p class="text-teal-100/90 text-sm font-medium max-w-md leading-relaxed">
                ...
                <div class="flex gap-3 shrink-0">
                    ...
                    <a href="{{ route('jovens.bible.favorites') }}" class="inline-flex items-center gap-2 rounded-2xl bg-white text-teal-900 px-4 py-2.5 text-xs font-semibold hover:bg-teal-50 transition-colors">
        ...
        <section class="jovens-bible-paper rounded-3xl border border-stone-200/90 dark:border-stone-800 p-6 md:p-8 shadow-sm">
            @if($chapters->isEmpty())
                <div class="text-center py-16">
                    ...
                    <h2 class="text-lg font-bold text-stone-900 dark:text-white">Sem capítulos</h2>
                    <p class="mt-2 text-sm text-stone-600 dark:text-stone-400 max-w-md mx-auto">
                        Esta tradução ainda não tem este livro carregado. Experimenta outra versão no leitor ou contacta a equipa JUBAF.
                    </p>
                    <a href="{{ route('jovens.bible.read', $version->abbreviation) }}" class="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-teal-700 dark:text-teal-400 hover:underline">
                        <x-icon name="arrow-left" class="w-4 h-4" />
                ...
                <div class="grid grid-cols-5 sm:grid-cols-7 md:grid-cols-9 lg:grid-cols-10 xl:grid-cols-12 gap-2">
                    @foreach($chapters as $chapter)
                        <a href="{{ route('jovens.bible.chapter', ['version' => $version->abbreviation, 'book' => $book->book_number, 'chapter' => $chapter->chapter_number]) }}"
                           class="group flex aspect-square flex-col items-center justify-center rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-900 text-stone-800 dark:text-stone-100 text-sm font-bold hover:border-teal-500 hover:bg-teal-50 dark:hover:bg-teal-950/50 hover:text-teal-900 dark:hover:text-teal-100 transition-colors">
