...
@section('content')
    ...
    <div class="mb-6 space-y-4 -mt-1 max-w-[88rem] mx-auto">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-3xl border border-teal-200/80 dark:border-teal-900/50 bg-gradient-to-r from-teal-50/95 via-white to-violet-50/40 dark:from-teal-950/40 dark:via-stone-900 dark:to-violet-950/20 px-5 py-4 shadow-sm">
            <div class="flex gap-3 min-w-0">
                <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-teal-600 text-white shadow-md shadow-teal-600/20">
                    <x-icon name="layer-group" class="w-5 h-5" />
                </span>
                <div class="min-w-0">
                    <p class="text-sm font-bold text-stone-900 dark:text-stone-50">Estudo interlinear no Unijovem</p>
                    <p class="text-xs text-stone-600 dark:text-stone-400 mt-0.5 leading-relaxed">
                        Texto original, Strong e comparação de versões — <strong class="text-stone-800 dark:text-stone-200">sem sair do painel de jovens</strong>. O botão «voltar» no leitor leva ao leitor por capítulos.
                    </p>
                </div>
            </div>
            <div class="flex flex-wrap gap-2 shrink-0">
                <a href="{{ route('jovens.bible.read') }}"
                   class="inline-flex items-center gap-2 rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 px-4 py-2.5 text-xs font-semibold text-stone-700 dark:text-stone-200 hover:border-teal-400/60 transition-colors">
                    <x-icon name="book-open" class="w-4 h-4 text-teal-600" />
                    Leitor por capítulo
                </a>
                <a href="{{ route('jovens.bible.search') }}"
                   class="inline-flex items-center gap-2 rounded-2xl bg-teal-600 hover:bg-teal-700 text-white px-4 py-2.5 text-xs font-semibold shadow-md shadow-teal-600/15 transition-colors">
