...
<style>
    ...
    @media (min-width: 1024px) {
        .floating-toolbar:not(.hidden) { bottom: 2rem; }
    }
...
<div class="bible-reader-container" id="reader-root">
    ...
    <header class="sticky top-0 z-40 bg-[var(--parchment-bg)] dark:bg-[#1a1a17] border-b border-[var(--sacred-gold)] px-6 py-4 flex items-center justify-between">
        <a href="{{ route('jovens.bible.plans.index') }}" class="text-[var(--sacred-red)] dark:text-[var(--sacred-gold)] inline-flex items-center gap-2 rounded-xl px-2 py-1 hover:bg-black/5 dark:hover:bg-white/5 transition-colors" title="Voltar aos planos">
            <x-icon name="chevron-left" class="w-6 h-6" />
            <span class="hidden sm:inline text-xs font-bold uppercase tracking-wider">Planos</span>
        </a>
    ...
    <main class="max-w-4xl mx-auto px-6 py-12 pb-32">
        @if($subscription->plan->reading_mode === 'digital')
                        ...
                        <div class="scripture-block">
                            <div class="flex items-center justify-between gap-4 mb-8">
                                <div class="flex items-center gap-4">
                                    ...
                                    <a href="{{ route('jovens.bible.chapter', ['version' => $targetVersion->abbreviation, 'book' => $content->target_book_id, 'chapter' => $content->chapter_start]) }}"
                                       class="text-[10px] font-black text-[var(--sacred-gold)] uppercase tracking-[0.2em] border border-[var(--sacred-gold)] px-2 py-1 rounded hover:bg-[var(--sacred-gold)] hover:text-white transition-all flex items-center gap-1">
                                        Abrir no leitor <x-icon name="book-open" class="w-3 h-3" />
                                    </a>
                                </div>
            ...
            <div class="mt-20 border-t-2 border-[var(--sacred-gold)] pt-12 text-center" id="reader-footer"
                 x-data="readingCheckIn({{ $isCompleted ? 'true' : 'false' }}, '{{ route('jovens.bible.reader.complete', [$subscription->id, $day->id]) }}', '{{ route('jovens.bible.reader.uncomplete', [$subscription->id, $day->id]) }}', '{{ csrf_token() }}')">
        ...
        @else
            ...
            <div x-data="readingTimer()" class="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-8">
                ...
                <div class="mt-12">
                    <a href="{{ route('jovens.bible.plans.pdf', $subscription->plan_id) }}" target="_blank" class="inline-flex items-center px-6 py-3 border-2 border-[var(--sacred-gold)] text-[var(--sacred-gold)] font-bold rounded-lg hover:bg-[var(--sacred-gold)] hover:text-white transition-all gap-2">
                        <x-icon name="document-download" class="w-5 h-5" />
                        Baixar Checklist (PDF)
                    </a>
                </div>

                <form x-ref="timerForm" action="{{ route('jovens.bible.reader.complete', [$subscription->id, $day->id]) }}" method="POST" class="hidden">
                    @csrf
