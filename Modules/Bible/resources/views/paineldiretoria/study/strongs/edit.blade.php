...
@section('content')
    <x-bible::admin.layout
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 space-y-4 shadow-sm">
            <div>
        ...
        <form action="{{ bible_admin_route('study.strongs.update', $entry->strong_number) }}" method="POST" class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 space-y-6 shadow-sm">
            @csrf
            @method('PUT')
            ...
            <div class="flex gap-3">
                <button type="submit" class="px-6 py-3 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-semibold shadow-sm transition-colors">Guardar</button>
            </div>
        </form>
        </div>
    </x-bible::admin.layout>
