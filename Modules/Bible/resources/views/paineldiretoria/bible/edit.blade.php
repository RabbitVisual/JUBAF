...
@section('title', 'Bíblia digital — editar ' . $version->name)

@section('content')
    <x-bible::admin.layout
        title="Editar versão: {{ $version->name }}"
        subtitle="Nome, idioma, ativação, modelo de URL de áudio e ligação à gestão de ficheiros por capítulo.">
        <x-slot name="actions">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
                    <x-icon name="pen-to-square" style="duotone" class="w-5 h-5 mr-2 text-amber-800 dark:text-amber-300" />
                    Informações da Versão
                </h2>
            </div>

            <form action="{{ bible_admin_route('update', $version) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <p class="mt-2">
                            <a href="{{ bible_admin_route('chapter-audio.index', $version) }}" class="text-sm font-medium text-blue-600 dark:text-blue-400 hover:underline inline-flex items-center gap-1">
                ...
                <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 flex items-center justify-end space-x-3">
                    ...
                    <button type="submit"
                        class="px-6 py-2.5 text-sm font-semibold text-white bg-amber-600 hover:bg-amber-700 rounded-xl shadow-sm transition-colors">
                        Guardar alterações
                    </button>
                </div>
            </form>
