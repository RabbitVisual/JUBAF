...
@section('content')
    <x-bible::admin.layout
        title="Relatório — plano da igreja"
        subtitle="Quem aderiu ao plano oficial da igreja, leitura concluída e atrasos (painel executivo).">
        <x-slot name="actions">
            <a href="{{ bible_admin_route('plans.index') }}"
                class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-gray-800 dark:text-gray-100 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                <x-icon name="arrow-left" class="w-5 h-5 shrink-0" />
                Planos de leitura
            </a>
        </x-slot>

        @if($churchPlans->isEmpty())
            <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-8 text-center shadow-sm">
                <x-icon name="book-bible" class="w-12 h-12 text-gray-400 dark:text-gray-500 mx-auto mb-3" />
                <p class="text-gray-600 dark:text-gray-400">Nenhum plano marcado como &quot;Plano da Igreja&quot;. Defina um plano como oficial em <a href="{{ bible_admin_route('plans.index') }}" class="text-blue-600 dark:text-blue-400 hover:underline">Planos de Leitura</a>.</p>
            </div>
        @else
            {{-- Métricas de conclusão --}}
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2 font-serif">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center gap-2 bg-gray-50 dark:bg-slate-900/50">
