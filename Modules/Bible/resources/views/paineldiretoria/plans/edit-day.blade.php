...
@section('content')
<x-bible::admin.layout :showNav="false" title="Editar dia — {{ $day->title }}" subtitle="Adicione leituras, vídeos ou devocionais a este dia do plano «{{ $plan->title }}».">
<div class="min-h-screen bg-gray-50/80 dark:bg-slate-900/40 pb-20 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">

    <!-- Top Bar -->
    <div class="sticky top-0 z-30 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border-b border-gray-200 dark:border-slate-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <a href="{{ bible_admin_route('plans.show', $plan->id) }}" class="p-2 -ml-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="submit" form="editorForm" id="topSaveBtn" class="px-6 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl shadow-sm text-sm font-bold transition-colors flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                                ...
                                <div class="md:w-1/2 {{ $loop->even ? 'md:ml-auto md:pl-8' : 'md:mr-auto md:pr-8' }}">
                                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 hover:shadow-md transition-shadow relative overflow-hidden cursor-pointer" onclick='editItem(@json($content))'>
                                        ...
                                        <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-100 p-1">
                                            ...
                                            <form action="{{ bible_admin_route('plans.content.destroy', $content->id) }}" method="POST" onsubmit="return confirm('Remover item?'); event.stopPropagation();" class="inline">
                                                @csrf @method('DELETE')
            ...
            <div class="lg:col-span-5">
                <div class="sticky top-24 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden ring-1 ring-gray-900/5">
                    ...
                    <div class="p-6">
                        <form action="{{ bible_admin_route('plans.content.store', $day->id) }}" method="POST" id="editorForm">
                            @csrf
...
<script>
    ...
    async function loadBooks(selectedId = null) {
        ...
        try {
            const res = await fetch(`{{ bible_admin_route('api.books') }}?version_id=${elements.versionSelect.value}`);
    ...
    async function loadChapters(selectedNum = null) {
        ...
        try {
            const res = await fetch(`{{ bible_admin_route('api.chapters') }}?book_id=${elements.bookSelect.value}`);
            const data = await res.json();
    ...
    async function loadVerses(startVal = null, endVal = null) {
        ...
        try {
            const res = await fetch(`{{ bible_admin_route('api.verses') }}?chapter_id=${option.dataset.id}`);
    ...
    async function editItem(item) {
        ...
        // Config Form
        const updateUrl = "{{ bible_admin_route('plans.content.update', 'ID') }}".replace('ID', item.id);
    ...
    function resetForm() {
        ...
        // Form Reset
        elements.form.action = "{{ bible_admin_route('plans.content.store', $day->id) }}";
