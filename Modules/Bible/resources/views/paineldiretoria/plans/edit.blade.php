...
@section('content')
    <x-bible::admin.layout
        title="Editar plano: {{ $plan->title }}"
        subtitle="Título, descrição, duração, publicação, navegação entre dias e imagem de capa.">
        <x-slot name="actions">
        ...
        <form action="{{ bible_admin_route('plans.update', $plan->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="space-y-8">
                    ...
                    <!-- Submit Action -->
                     <button type="submit" class="w-full py-4 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl shadow-sm transition-colors inline-flex items-center justify-center gap-2">
                        Salvar alterações
