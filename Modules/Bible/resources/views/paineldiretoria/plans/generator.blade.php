...
@section('title', 'Bíblia digital — gerar plano')

@section('content')
    <x-bible::admin.layout
        title="Gerador do plano"
        subtitle="Escolha a versão bíblica e os livros; o sistema reparte capítulos pelos {{ $plan->duration_days }} dias.">
        <x-slot name="actions">
            <a href="{{ bible_admin_route('plans.show', $plan->id) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 text-sm font-semibold text-gray-800 dark:text-gray-100 bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                Voltar ao plano
            </a>
        </x-slot>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
        ...
        <form action="{{ bible_admin_route('plans.process-generation', $plan->id) }}" method="POST">
            @csrf

            @if($plan->type !== 'manual')
            {{-- Versão obrigatória para geração automática (não aplica a planos só com dias vazios). --}}
            <div class="mb-8 p-6 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <select name="bible_version_id" id="versionSelector" onchange="filterBooksByVersion()" class="w-full md:w-1/3 px-4 py-3 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:ring-blue-500 focus:border-blue-500 transition-colors">
            ...
            @if($plan->type === 'manual')
                <div class="text-center py-12">
                    ...
                    <button type="submit" class="inline-flex items-center px-8 py-4 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl shadow-sm transition-colors">
            ...
            @else
                {{-- sequential, thematic: escolha de livros --}}
