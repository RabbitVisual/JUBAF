...
@section('title', 'Bíblia digital — novo plano')

@section('content')
    <x-bible::admin.layout
        title="Novo plano de leitura"
        subtitle="Tipo (manual, livros ou Bíblia completa), modo digital ou físico, duração e capa. Depois gera ou edita os dias.">
        <x-slot name="actions">
            <a href="{{ bible_admin_route('plans.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 text-sm font-semibold text-gray-800 dark:text-gray-100 bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                <x-icon name="arrow-left" class="w-4 h-4 shrink-0" />
                Lista de planos
            </a>
        </x-slot>

        <form action="{{ bible_admin_route('plans.store') }}" method="POST" enctype="multipart/form-data"
              x-data="{
                  days: 7,
                  type: 'sequential',
                  get estimate() {
                      ...
                      if (this.type === 'thematic') return 'Distribui os livros que escolher pelos dias, alinhados ao tema pastoral definido na descrição.';
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <!-- Type: Thematic -->
                            <label class="cursor-pointer relative group md:col-span-3">
                                <input type="radio" name="type" value="thematic" class="peer sr-only" x-model="type">
                                <div class="h-full p-5 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30 hover:border-amber-300 dark:hover:border-amber-500 peer-checked:border-amber-600 peer-checked:bg-amber-50 dark:peer-checked:bg-amber-900/10 transition-all flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                    <div class="flex gap-3">
                                        <div class="w-10 h-10 shrink-0 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 flex items-center justify-center">
                                            <x-icon name="tags" class="w-6 h-6" />
                                        </div>
                                        <div>
                                            <h3 class="font-bold text-gray-900 dark:text-white mb-1">Temático / série</h3>
                                            <p class="text-xs text-gray-500 dark:text-gray-400 leading-normal">Escolha livros ou trechos alinhados a um tema (ex.: aliança, templo, reino). Na etapa seguinte seleciona livros e a versão bíblica.</p>
                                        </div>
                                    </div>
                                    <div class="text-amber-600 dark:text-amber-400 opacity-0 peer-checked:opacity-100 transition-opacity flex justify-end sm:shrink-0">
                                        <x-icon name="check-circle" class="w-6 h-6" />
                                    </div>
                                </div>
                            </label>
                ...
                <div class="space-y-8">
                    ...
                    <!-- Editorial / pastoral options -->
                    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
                        <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Opções editoriais</h2>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Visibilidade, uso em igreja e nível de exigência de leitura.</p>
                        <div class="space-y-4">
                            <label class="flex items-start gap-3 cursor-pointer">
                                <input type="checkbox" name="is_featured" value="1" class="mt-1 rounded border-gray-300 text-amber-600 focus:ring-amber-500">
                                <span class="text-sm text-gray-700 dark:text-gray-300"><span class="font-semibold">Destacar</span> na lista pública de planos (quando ativo).</span>
                            </label>
                            <label class="flex items-start gap-3 cursor-pointer">
                                <input type="checkbox" name="is_church_plan" value="1" class="mt-1 rounded border-gray-300 text-amber-600 focus:ring-amber-500">
                                <span class="text-sm text-gray-700 dark:text-gray-300"><span class="font-semibold">Plano de igreja</span> — campanha ou leitura conjunta (relatórios e filtros).</span>
                            </label>
                            <div>
                                <label for="plan_complexity_pd" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Complexidade sugerida</label>
                                <select id="plan_complexity_pd" name="complexity" class="w-full md:w-1/2 rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm">
                                    <option value="">— Não especificar —</option>
                                    <option value="light">Leve (poucos capítulos/dia)</option>
                                    <option value="moderate">Moderada</option>
                                    <option value="intense">Intensa (muitos capítulos ou estudo profundo)</option>
                                </select>
                            </div>
                            <div>
                                <label for="template_key_pd" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Chave de modelo interno (opcional)</label>
                                <input type="text" id="template_key_pd" name="template_key" maxlength="64" placeholder="ex.: cbav2026, advento"
                                    class="w-full md:w-2/3 rounded-xl border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm">
                                <p class="text-xs text-gray-500 mt-1">Para integrações e relatórios; não aparece ao leitor.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Submit Action -->
                     <button type="submit" class="w-full py-4 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl shadow-sm transition-colors flex items-center justify-center gap-2 group">
                        Continuar para configuração
                        <x-icon name="arrow-right" class="w-5 h-5 shrink-0 group-hover:translate-x-0.5 transition-transform" />
                    </button>
                </div>
            </div>
        </form>
