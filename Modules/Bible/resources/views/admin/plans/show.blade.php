...
@section('content')
    <x-bible::admin.layout
        title="{{ $plan->title }}"
        subtitle="{{ $plan->is_active ? 'Publicado' : 'Rascunho' }} · {{ $plan->reading_mode === 'physical_timer' ? 'Físico (timer)' : 'Digital' }} · {{ $plan->duration_days }} dias · {{ $plan->days_count }} dias gerados">
        <x-slot name="actions">
            <a href="{{ bible_admin_route('plans.index') }}" class="px-4 py-2.5 border border-amber-200/80 dark:border-amber-900/50 rounded-xl text-sm font-semibold text-stone-800 dark:text-stone-100 bg-white dark:bg-stone-800 hover:bg-amber-50 dark:hover:bg-stone-700 transition">Voltar</a>
            <a href="{{ bible_admin_route('plans.edit', $plan->id) }}" class="px-4 py-2.5 bg-white dark:bg-stone-800 border border-amber-200/80 dark:border-amber-900/50 rounded-xl text-sm font-semibold text-stone-800 dark:text-stone-100 hover:bg-amber-50 dark:hover:bg-stone-700 transition flex items-center">
                <x-icon name="pen-to-square" style="duotone" class="w-4 h-4 mr-2" />
                Editar
            </a>
            <a href="{{ bible_admin_route('plans.generate', $plan->id) }}" class="px-4 py-2.5 bg-linear-to-r from-amber-800 to-stone-900 hover:from-amber-900 hover:to-stone-950 text-amber-50 rounded-xl transition shadow-md flex items-center text-sm font-semibold" onclick="return {{ $plan->days_count > 0 ? 'confirm(\'Isso irá apagar todo o conteúdo atual e permitir gerar novamente. Continuar?\')' : 'true' }}">
