...
@section('content')
    <x-bible::admin.layout
        title="Importar versão da Bíblia"
        subtitle="Envie um arquivo JSON no formato esperado para disponibilizar uma nova tradução aos membros.">
        <x-slot name="actions">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-amber-200/70 dark:border-amber-900/45 overflow-hidden">
            ...
            <form action="{{ bible_admin_route('import.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 flex items-center justify-end space-x-3">
                    ...
                    <button type="submit"
                        class="px-6 py-2.5 text-sm font-semibold text-amber-50 bg-linear-to-r from-amber-800 to-stone-900 hover:from-amber-900 hover:to-stone-950 rounded-xl shadow-md transition-all flex items-center">
        ...
        <!-- Format Info Card -->
        <div class="bg-linear-to-r from-amber-50/90 to-stone-100/80 dark:from-amber-950/40 dark:to-stone-900/50 border border-amber-200/70 dark:border-amber-900/45 rounded-2xl p-6">
            <h3 class="text-sm font-semibold text-amber-950 dark:text-amber-200 mb-3 flex items-center">
