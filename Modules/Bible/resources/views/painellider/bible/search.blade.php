...
@section('breadcrumbs')
    ...
    <span class="text-slate-400">/</span>
    <span class="text-slate-600 dark:text-slate-300">Busca</span>
@endsection
    ...
    <div class="sticky top-0 z-40 bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl border-b border-gray-200 dark:border-slate-800 shadow-sm pt-4 pb-6 px-4 transition-colors duration-200">
        <div class="max-w-3xl mx-auto space-y-4">
            ...
            <div class="flex items-center justify-between">
                <a href="{{ route('lideres.bible.index') }}" class="group inline-flex items-center text-xs font-bold text-gray-500 hover:text-emerald-600 dark:text-slate-400 dark:hover:text-emerald-400 uppercase tracking-widest transition-colors">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <h1 class="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">Concordância</h1>
                </div>
            </div>
            ...
            <div class="relative group" data-tour="bible-search-input">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
                   <x-icon name="magnifying-glass" class="w-5 h-5 text-gray-400 dark:text-slate-500 group-focus-within:text-emerald-500 transition-colors" />
                </div>
                ...
                <input type="text"
                       ...
                       class="w-full bg-gray-100 dark:bg-slate-900 border-2 border-transparent focus:border-emerald-500 dark:focus:border-emerald-500 text-gray-900 dark:text-white text-lg font-bold rounded-2xl py-4 pl-12 pr-12 shadow-inner focus:shadow-xl focus:shadow-emerald-500/10 focus:bg-white dark:focus:bg-slate-900 transition-all placeholder:font-medium placeholder:text-gray-400 dark:placeholder:text-slate-600 outline-none">

                <!-- Loading Spinner -->
                <div x-show="loading" class="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-500">
        ...
        <div class="space-y-4" data-tour="bible-search-results">
            <template x-for="verse in results" :key="verse.id">
                <a :href="chapterUrl(verse)"
                   class="block bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-100 dark:border-slate-800 hover:border-emerald-500 dark:hover:border-emerald-500 shadow-sm hover:shadow-xl hover:shadow-emerald-500/10 transition-all group relative overflow-hidden">

                    <!-- Decorative Hover Gradient -->
                    <div class="absolute inset-0 bg-linear-to-r from-emerald-500/0 via-emerald-500/0 to-emerald-500/5 translate-x-full group-hover:translate-x-0 transition-transform duration-500 pointer-events-none"></div>

                    <!-- Header: Reference -->
                    <div class="flex justify-between items-center mb-4 relative z-10">
                        <div class="flex items-center gap-3">
                             <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 font-black text-xs">
                    ...
                    <p class="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mt-1">
                        ...
                        <div class="w-8 h-8 rounded-full bg-gray-50 dark:bg-slate-800 flex items-center justify-center group-hover:bg-emerald-500 group-hover:text-white transition-colors duration-300">
    ...
    function bibleSearch(opts = {}) {
        return {
            ...
            async performSearch() {
                ...
                try {
                    const response = await fetch(`{{ route('lideres.bible.api.search') }}?q=${encodeURIComponent(this.query)}`);
