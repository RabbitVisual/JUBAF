...
@section('breadcrumbs')
    ...
    <span class="text-slate-400">/</span>
    <a href="{{ route('lideres.bible.read', $version->abbreviation) }}" class="text-emerald-600 dark:text-emerald-400 hover:underline">{{ $version->abbreviation }}</a>
    <span class="text-slate-400">/</span>
...
@section('content')
    <div class="min-h-screen bg-white dark:bg-slate-950 transition-colors pb-20" id="bible-chapter" style="--bible-mobile-font-size: 1.25rem">
        ...
        <nav class="sticky top-0 z-40 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md border-b border-gray-100 dark:border-slate-800 transition-colors" data-tour="bible-chapter-nav">
            <div class="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
                <!-- Back to Book -->
                <a href="{{ route('lideres.bible.book', ['version' => $version->abbreviation, 'book' => $book->book_number]) }}"
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    <h1 class="text-lg sm:text-xl font-black text-gray-900 dark:text-white tracking-tight">
                        {{ $book->name }} <span class="text-emerald-600 dark:text-emerald-400">{{ $chapter->chapter_number }}</span>
                    </h1>
                    <div class="h-6 w-px bg-gray-200 dark:bg-slate-700"></div>
                     <div class="relative group">
                        <select onchange="changeVersion(this.value)"
                                class="appearance-none bg-transparent text-sm font-bold text-gray-600 dark:text-slate-300 pr-6 outline-none cursor-pointer hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
                ...
                <div class="flex items-center gap-2">
                     <a href="{{ route('lideres.bible.interlinear', ['book' => $book->name, 'chapter' => $chapter->chapter_number]) }}"
                        class="hidden sm:inline-flex items-center justify-center px-3 py-1.5 text-xs font-bold text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">
                        <x-icon name="language" class="w-3 h-3 mr-1.5" />
                        Interlinear
                    </a>
                    <button type="button" id="bible-font-size-btn" class="sm:hidden p-2 text-gray-400 hover:text-emerald-600 dark:text-slate-500 dark:hover:text-emerald-400 touch-manipulation active:scale-95" title="Aumentar tamanho da fonte" aria-label="Aumentar tamanho da fonte">
            ...
            @if (!empty($chapterAudioUrl))
                <div class="mb-8 p-4 rounded-xl bg-gray-50 dark:bg-slate-900/50 border border-gray-100 dark:border-slate-800" aria-label="Ouvir capítulo em áudio">
                    <p class="text-sm font-bold text-gray-600 dark:text-slate-400 mb-3 flex items-center gap-2">
                        <x-icon name="volume-high" class="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                    ...
                    @foreach ($verses as $verse)
                        <div class="flex items-start gap-3 sm:gap-5 group relative p-2 sm:p-3 -mx-2 sm:-mx-3 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-900/50 transition-colors duration-200" id="verse-{{ $verse->verse_number }}">
                            ...
                            <div class="md:opacity-0 md:group-hover:opacity-100 flex flex-col gap-1 transition-opacity duration-200 absolute right-2 top-2 sm:static">
                                ...
                                <button type="button"
                                    class="verse-share-btn p-2 rounded-lg text-gray-300 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors"
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-950/90 backdrop-blur-lg border-t border-gray-200 dark:border-slate-800 py-4 px-6 z-30">
             <div class="max-w-4xl mx-auto flex items-center justify-between gap-4">
                ...
                <div class="flex-1">
                    @if ($previousChapter)
                        <a href="{{ route('lideres.bible.chapter', ['version' => $version->abbreviation, 'book' => $previousChapter->book->book_number, 'chapter' => $previousChapter->chapter_number]) }}"
                            class="flex items-center gap-3 text-gray-500 hover:text-gray-900 dark:text-slate-400 dark:hover:text-white transition-colors group">
                             <div class="w-10 h-10 rounded-xl bg-gray-100 dark:bg-slate-800 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/20 flex items-center justify-center transition-colors">
                                 <x-icon name="chevron-left" class="w-5 h-5 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
                             </div>
                ...
                <div class="sm:hidden text-center">
                    <span class="text-xs font-black uppercase tracking-widest text-emerald-600">{{ $book->abbreviation }} {{ $chapter->chapter_number }}</span>
                </div>

                <!-- Next -->
                 <div class="flex-1 flex justify-end">
                    @if ($nextChapter)
                        <a href="{{ route('lideres.bible.chapter', ['version' => $version->abbreviation, 'book' => $nextChapter->book->book_number, 'chapter' => $nextChapter->chapter_number]) }}"
                            class="flex items-center gap-3 text-gray-500 hover:text-gray-900 dark:text-slate-400 dark:hover:text-white transition-colors group text-right">
                             ...
                             <div class="w-10 h-10 rounded-xl bg-gray-100 dark:bg-slate-800 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/20 flex items-center justify-center transition-colors">
                                 <x-icon name="chevron-right" class="w-5 h-5 group-hover:text-emerald-600 dark:group-hover:text-emerald-400" />
                             </div>
                        </a>
                    @endif
                </div>
    ...
    <script>
        ...
        function toggleFavorite(verseId) {
            ...
            fetch(`{{ url('/lideres/biblia/favorites') }}/${verseId}`, {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            @php
                $favoriteIds = \Modules\Bible\App\Models\BibleFavorite::where('user_id', Auth::id())->pluck('verse_id')->toArray();
            @endphp
