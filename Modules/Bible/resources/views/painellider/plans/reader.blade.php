...
@section('breadcrumbs')
    ...
    <span class="text-slate-400">/</span>
    <span class="text-slate-600 dark:text-slate-300 line-clamp-1">{{ $subscription->plan->title }} — dia {{ $day->day_number }}</span>
@endsection
...
<div class="bible-reader-container" id="reader-root">
    ...
    <header class="sticky top-0 z-40 bg-[var(--parchment-bg)] dark:bg-[#1a1a17] border-b border-[var(--sacred-gold)] px-6 py-4 flex items-center justify-between">
        <a href="{{ route('lideres.bible.plans.index') }}" class="text-[var(--sacred-red)] dark:text-[var(--sacred-gold)]">
    ...
    <main class="max-w-4xl mx-auto px-6 py-12 pb-32">
        @if($subscription->plan->reading_mode === 'digital')
                        ...
                        <div class="scripture-block">
                            <div class="flex items-center justify-between gap-4 mb-8">
                                <div class="flex items-center gap-4">
                                    ...
                                    <a href="{{ route('lideres.bible.chapter', ['version' => $targetVersion->abbreviation, 'book' => $content->target_book_id, 'chapter' => $content->chapter_start]) }}"
                                       target="_blank"
            ...
            <div class="mt-20 border-t-2 border-[var(--sacred-gold)] pt-12 text-center" id="reader-footer"
                 x-data="readingCheckIn({{ $isCompleted ? 'true' : 'false' }}, '{{ route('lideres.bible.reader.complete', [$subscription->id, $day->id]) }}', '{{ route('lideres.bible.reader.uncomplete', [$subscription->id, $day->id]) }}', '{{ csrf_token() }}')">
        ...
        @else
            ...
            <div x-data="readingTimer()" class="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-8">
                ...
                <div class="mt-12">
                    <a href="{{ route('lideres.bible.plans.pdf', $subscription->plan_id) }}" target="_blank" class="inline-flex items-center px-6 py-3 border-2 border-[var(--sacred-gold)] text-[var(--sacred-gold)] font-bold rounded-lg hover:bg-[var(--sacred-gold)] hover:text-white transition-all gap-2">
                        <x-icon name="document-download" class="w-5 h-5" />
                        Baixar Checklist (PDF)
                    </a>
                </div>

                <form x-ref="timerForm" action="{{ route('lideres.bible.reader.complete', [$subscription->id, $day->id]) }}" method="POST" class="hidden">
                    @csrf
    ...
    (function() {
        ...
        */
        ...
        async function batchSend(color, note = '', type = 'highlight') {
            ...
            try {
                const response = await fetch(@json(route('lideres.bible.favorites.batch')), {
                    method: 'POST',
                    headers: {
