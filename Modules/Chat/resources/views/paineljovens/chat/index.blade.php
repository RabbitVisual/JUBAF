...
@section('jovens_content')
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p id="chat-header-status"
                                class="text-xs font-medium text-emerald-600 dark:text-emerald-400 mt-0.5">Conversação ativa
                            </p>
                        </div>
                    </div>
                </div>
    ...
    @push('scripts')
        @php
            $messagesUrlTpl = route('jovens.chat.messages', ['sessionId' => '__SID__']);
            $sendUrlTpl = route('jovens.chat.send', ['sessionId' => '__SID__']);
        @endphp
        <script>
            ...
            async function carregarConversas() {
                try {
                    const res = await fetch('{{ route('jovens.chat.index') }}', {
                        headers: {
                    ...
                    if (data.success) {
                        ...
                        if (!data.messages.length) {
                            ...
                            container.innerHTML = data.messages.map(m => {
                                ...
                                const label = isMe ? 'Você' : (m.sender?.name || 'Equipa JUBAF');
                                return `
            ...
            async function abrirNovaConversa() {
                ...
                try {
                    const res = await fetch('{{ route('jovens.chat.users') }}', {
                        headers: {
                            'Accept': 'application/json'
                        }
                    });
            ...
            document.getElementById('form-nova-conversa').addEventListener('submit', async (e) => {
                ...
                btn.disabled = true;
                try {
                    const res = await fetch('{{ route('jovens.chat.store') }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
