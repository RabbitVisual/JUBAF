...
<div class="space-y-8 md:space-y-10 animate-fade-in pb-8">
    <a href="{{ route($routePrefix.'.index') }}" class="inline-flex items-center gap-2 text-sm font-bold text-violet-700 hover:text-violet-800 dark:text-violet-400 dark:hover:text-violet-300 transition-all hover:gap-2.5">
        <x-icon name="arrow-left" class="h-4 w-4" style="duotone" />
        Voltar aos eventos
    </a>

    <article class="overflow-hidden rounded-[2rem] border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-800 shadow-2xl shadow-violet-900/10">
        <div class="relative overflow-hidden border-b border-white/10 bg-gradient-to-br from-violet-600 via-fuchsia-600 to-slate-900 px-8 py-10 md:px-12 text-white">
            ...
            <div class="relative">
                <p class="text-xs font-bold uppercase tracking-wider text-violet-100/90">{{ $event->starts_at->format('d/m/Y · H:i') }}</p>
                <h1 class="mt-3 text-2xl md:text-3xl font-bold tracking-tight leading-tight">{{ $event->title }}</h1>
                @if($event->type)
                    <p class="mt-2 text-sm font-medium capitalize text-fuchsia-100/95">{{ $event->type }}</p>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-6 px-6 py-8 md:px-10 md:py-10">
            @if($event->ends_at)
                <p class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <x-icon name="clock" class="h-4 w-4 text-violet-500 dark:text-violet-400" style="duotone" />
            ...
            @if($event->location)
                <p class="flex items-start gap-3 text-gray-700 dark:text-gray-300">
                    <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300">
            ...
            @if($event->description)
                <div class="rounded-2xl border border-violet-100 bg-violet-50/40 px-5 py-4 dark:border-violet-900/40 dark:bg-violet-950/20">
            ...
            @elseif($event->registration_open && $event->isRegistrationPeriodOpen())
                <form action="{{ route($routePrefix.'.register', $event) }}" method="post" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="discount_code" value="{{ old('discount_code') }}" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-white" placeholder="Se tiveres um código">
                    </div>
                    <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 px-8 py-4 text-sm font-bold text-white shadow-xl shadow-violet-600/30 transition hover:brightness-110 active:scale-[0.99] sm:w-auto">
            ...
            @if(!empty($hasPublicCalendar))
                <div class="border-t border-gray-100 pt-6 dark:border-slate-700">
                    <a href="{{ route('eventos.index') }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 text-sm font-bold text-violet-700 hover:text-violet-900 dark:text-violet-400 dark:hover:text-violet-200">
