...
@section('content')
    @component('memberpanel::components.institutional-page', [
        ...
        <form method="post" action="{{ route('memberpanel.governance.communications.update', $communication) }}"
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $communication->title) }}" required
            ...
            <div>
                ...
                <textarea name="summary" rows="2"
                    class="w-full rounded-xl border-gray-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white">{{ old('summary', $communication->summary) }}</textarea>
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">Corpo</label>
                <textarea name="body" rows="12" required
                    class="w-full rounded-xl border-gray-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white">{{ old('body', $communication->body) }}</textarea>
            </div>
            <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                <input type="hidden" name="is_published" value="0">
                <input type="checkbox" name="is_published" value="1" @checked(old('is_published', $communication->is_published))
            ...
            <div class="flex flex-wrap gap-3 pt-2">
                ...
                <a href="{{ route('memberpanel.governance.communications.show', $communication) }}"
