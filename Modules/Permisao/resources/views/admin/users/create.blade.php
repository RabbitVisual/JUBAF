...
<div class="space-y-6 max-w-4xl">
    <div class="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-slate-700">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Criar utilizador</h1>
    ...
    <form action="{{ route($routePrefix.'.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 space-y-3">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400">Para Vice-Presidentes: o setor limita igrejas, secretaria e tesouraria no painel da diretoria. Opcional.</p>
            <div>
                ...
                <select name="jubaf_sector_id" id="jubaf_sector_id"
                    ...
                    @foreach($jubafSectors as $sec)
                        <option value="{{ $sec->id }}" @selected(old('jubaf_sector_id') == $sec->id)>{{ $sec->name }}</option>
                    @endforeach
                </select>
