...
<head>
    ...
    <meta name="apple-mobile-web-app-title" content="JUBAF Líderes">
    <meta name="mobile-web-app-capable" content="yes">
    <meta name="application-name" content="JUBAF — Painel de Líderes">
    <meta name="msapplication-TileColor" content="#047857">
    <meta name="msapplication-tap-highlight" content="no">

    <title>@yield('title', 'Painel de Líderes') — JUBAF</title>
...
<body class="h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col selection:bg-emerald-500/20 selection:text-emerald-700 overflow-x-hidden" x-data="{ sidebarOpen: false }">
    ...
    <div id="global-loading" class="fixed inset-0 z-[100] flex items-center justify-center bg-white/80 dark:bg-slate-950/80 backdrop-blur-md transition-opacity duration-300 pointer-events-none opacity-0">
        <div class="flex flex-col items-center">
            ...
            <p class="mt-4 text-xs font-semibold text-emerald-600 animate-pulse">A carregar…</p>
        </div>
    </div>
            ...
            <main class="flex-1 overflow-y-auto overflow-x-hidden p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8 text-[15px] leading-relaxed text-slate-800 dark:text-slate-200">
                <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs sm:text-sm font-semibold text-slate-500 dark:text-slate-400 mb-1">
                    <span class="inline-flex items-center gap-1.5 shrink-0">
                        <x-icon name="users" class="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                        <span class="text-slate-600 dark:text-slate-300">Painel de líderes</span>
                    </span>
                    @yield('breadcrumbs')
                </div>

                @if (session('success'))
                    <div class="rounded-2xl border border-emerald-200 dark:border-emerald-900/50 bg-emerald-50 dark:bg-emerald-950/40 px-4 py-3 text-sm font-semibold text-emerald-800 dark:text-emerald-200" role="status">
                        {{ session('success') }}
                    </div>
                @endif
