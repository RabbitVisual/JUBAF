<nav class="min-h-16 md:min-h-[4.5rem] lg:min-h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3 px-4 md:px-6 lg:px-8 py-3 z-30 sticky top-0">
    <div class="flex items-center gap-3 min-w-0 flex-1">
        <button type="button" @click="sidebarOpen = true" class="lg:hidden shrink-0 w-11 h-11 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-emerald-600 hover:text-white transition-all shadow-sm active:scale-95" aria-label="Abrir menu">
            <x-icon name="bars-staggered" style="duotone" class="w-5 h-5" />
        </button>

        <div class="hidden sm:block min-w-0">
            <p class="text-xs font-semibold text-emerald-700 dark:text-emerald-400 leading-snug truncate max-w-[14rem] md:max-w-none">
                {{ \App\Support\SiteBranding::siteName() }}
            </p>
            <div class="flex items-center gap-2 mt-0.5">
                <span class="w-2 h-2 rounded-full bg-emerald-500 shrink-0 animate-pulse shadow-[0_0_8px_theme(colors.emerald.500)]" aria-hidden="true"></span>
    ...
    <div class="hidden lg:flex flex-1 justify-center max-w-xl px-2 min-w-0">
        <div class="flex items-start gap-3 px-4 py-2.5 bg-slate-100/90 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 w-full">
            <x-icon name="bullhorn" class="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
            <p class="text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
                Área reservada a líderes de igrejas locais. Os dados da tua unidade serão integrados conforme a secretaria JUBAF atualizar o sistema.
            </p>
        </div>
    </div>
    ...
    <div class="flex items-center gap-2 md:gap-3 shrink-0">
        ...
        <button type="button"
            ...
            class="relative w-11 h-11 md:w-12 md:h-12 shrink-0 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all shadow-sm active:scale-95 group"
        ...
        <div class="relative" x-data="{ open: false }" @click.away="open = false">
            <button type="button" @click="open = !open" class="flex items-center gap-2 md:gap-3 p-1.5 md:pr-3 bg-slate-100 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 hover:border-emerald-500/40 transition-all active:scale-[0.98] group max-w-[220px]">
                <div class="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white text-sm font-bold shadow-md group-hover:rotate-3 transition-transform overflow-hidden shrink-0">
                ...
                <div class="hidden md:flex flex-col text-left min-w-0">
                    ...
                    <span class="text-xs text-slate-500 dark:text-slate-400 truncate">Líder local</span>
                </div>
            ...
            <div x-show="open"
                ...
                <div class="p-2 space-y-0.5">
                    <a href="{{ route('lideres.profile.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-700 dark:text-slate-300 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 hover:text-emerald-700 dark:hover:text-emerald-300 transition-colors group">
