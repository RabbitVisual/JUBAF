...
<head>
    ...
    <title>@yield('title', 'Pastor') — {{ \App\Support\SiteBranding::siteName() }}</title>
    <script>
        (function() {
...
<body class="h-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 antialiased">
    ...
    <main class="max-w-6xl mx-auto px-4 py-8">
        @if(session('success'))
            <div class="mb-6 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900 text-emerald-800 dark:text-emerald-200 text-sm font-medium">{{ session('success') }}</div>
        @endif
        @yield('content')
    </main>
</body>
